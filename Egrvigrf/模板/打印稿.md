...
struct LazySegmentTree {
    ...
    vector<Node> tree;
    vector<Tag> tag;
    LazySegmentTree(vector<Node>& a,int _n):n(_n),tree(4<<__lg(n)), tag(4<<__lg(n)) {
        build(a, 1, 1, _n);
    }
    void up(int p) {
        tree[p] = tree[p << 1] + tree[p << 1 | 1];
    }
    void build(vector<Node>& a, int p, int l, int r) {
        tag[p].len = r - l + 1;
        if (l == r) {
            tree[p] = a[l];
            return;
    ...
    void addlazy(int p, int v) {
        tree[p].sum += v * tag[p].len;
        tag[p].addV += v;
    }
    void down(int p) {
        if (tag[p].addV != 0) {
            addlazy(p << 1, tag[p].addV);
            addlazy(p << 1 | 1, tag[p].addV);
            tag[p].addV = 0;
        }
    }
    void add(int al, int ar, int v, int p, int l, int r) {
        ...
        int m = (l + r) >> 1;
        down(p);
        if (al <= m) add(al, ar, v, p << 1, l, m);
        if (ar > m) add(al, ar, v, p << 1 | 1, m + 1, r);
        up(p);
    }
    void add(int al, int ar, int v) {
        add(al, ar, v, 1, 1, n);
    }
...
signed main() {
    ...
    for (int i = 1; i <= m; i++) {
        ...
        if (op == 1) {
            ...
            cout << segtree.query(x, y).sum << endl;
        }
    }
    return 0;
}
